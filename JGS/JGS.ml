...
module HO = struct
  ...
  let (_ :
        (('a ilogic -> OCanren.goal) -> 'b ilogic -> OCanren.goal)
        -> ('a ilogic Std.List.injected -> OCanren.goal)
  ...
  let rec substitute_typ
    ...
    -> (jtype_injected -> goal) -> jtype_injected -> goal
    =
   fun subst q0 q30 ->
    fresh
      q3
  ...
  and substitute_arg
    :  (jtype_injected targ_injected Std.List.injected -> goal)
    -> (jtype_injected targ_injected -> goal) -> jtype_injected targ_injected -> goal
    =
   fun subst q34 q63 ->
    fresh
      q37
 ...
 ;;
  ...
  struct
    ...
    let rec capture_conversion
      :  ((jtype_injected -> Option.HO.goal)
          -> (jtype_injected -> Option.HO.goal)
          -> bool ilogic
          -> State.t
          -> State.t OCanren.Stream.t)
      -> (int ilogic -> OCanren.goal)
      -> (jtype_injected targ_injected Std.List.injected -> goal)
      -> jtype_injected targ_injected Std.List.injected option ilogic -> OCanren.goal
      =
     fun ( <-< ) id targs q205 ->
      (*
      if need_simpified
   ...
   ;;
    ...
    and class_int_sub
      :  ((jtype_injected -> Option.HO.goal)
          ...
          -> bool ilogic
          -> Option.HO.goal)
      -> int ilogic -> (jtype_injected targ_injected list_injected -> Option.HO.goal)
      -> (int ilogic -> Option.HO.goal)
      -> ((( ( jtype_injected
             , (polarity_injected, jtype_injected) Std.Pair.groundi option_injected )
             targ_fuly
             ilogic
           , 'a ilogic )
           Std.List.t
      ...
      -> bool ilogic -> goal
      =
     fun ( <-< ) id_a targs_a id_b targs_b q235 -> q235 === q235
    (* fresh
        (q233 q208)
        (id_b q233)
        (conde
           [ fresh () (id_a === q233) (q208 === !!true)
           ; fresh () (q208 === !!false) (id_a =/= q233)
           ])
        (conde
           [ fresh
               ()
           ...
           ; fresh
               q211
               (q208 === !!false)
               (CT.HO.get_superclass (( === ) id_a) (( === ) q233) q211)
               (conde
                  [ fresh
